...
**而且你不用走進去也看得到它在做什麼。** 選中正在跑的那則，預覽下半部就即時
照出那個 agent 的畫面 —— 上半部還是你寫的待辦，下半部是它此刻在幹嘛。
...
- **不用走進去就看得到。** 選中正在跑的那則，預覽下半部即時照出它的畫面 ——
  清單、你寫的內容、它此刻在做什麼，同一個畫面上
...
- **圖表** —— ```` ```mermaid ````（或 ```` ```plantuml ````）會畫成真的圖，用 awk：
  - `sequenceDiagram` / PlantUML 訊息 —— 生命線、箭頭、虛線回應、自我訊息迴圈
  - `flowchart` / `graph` —— Sugiyama 分層，含斷環與跨層邊的虛擬節點；
    平行邊合併標籤，回邊列成註腳

  兩種的中文寬度都正確。其他的（`erDiagram`、`C4Context`、`gantt`、`pie`…）
  退回程式碼區塊，所以最壞情況就是你現在看到的樣子
- **表格**對齊分欄（`│ ┼`），支援 `:---:` 與 `---:` 的對齊宣告。
...
| 鍵 | 動作 |
|---|---|
| *打字* | 篩選（子字串比對，中文可用）。也吃 window_id —— agent 講「@355」時打 `355` 就找得到 |
| `↑` `↓` | 移動選擇（`C-p` / `C-n` 同） |
| `C-e` `C-y` | 預覽捲一行 |
| `C-d` `C-u` | 預覽捲半頁 |
| `C-f` `C-b` | 預覽捲整頁 |
...
| `C-o` | 離開回工作區 —— 上次從「不是待辦」的 window 開清單的地方；沒有的話取這個 session 第一個不是待辦的 window |
| `prefix` + `⌥←` `⌥→` | 調整中間分隔線（tmux 原生綁定，寬度會記住） |

每一列開頭是那則的 **`window_id`**（`@355`，暗色），預覽的第一行也會再印一次。
agent 在對話裡是用這個 id 指某一則的（「@355 自己標它是本則最大的不確定性」）——
畫面上看不到就對不起來。id 欄只有最長那個那麼寬，而且寬度是照「全部待辦」算的，
不是照命中的，所以打字篩選時不會左右跳。
...
**markdown render 是 1,607 行 awk**（`md.awk` 加上 `width.awk` `graph.awk` `seq.awk` `flow.awk`）—— 標題、清單、行內程式碼、引用、
