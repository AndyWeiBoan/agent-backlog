...
# 寫內容。**所有寫入待辦內容的地方都要走這一支**，因為它是唯一會檢查失敗的地方。
#
# ⚠️ 內容有硬上限，而且不是我們訂的：tmux 對「一整條指令」有 16 KB 的長度限制，
# 而內容是當成 set-option 的參數傳過去的。實測在這台機器上，
# 值最多 16337 bytes（16384 減掉指令本身佔的字元），再多一個 byte 就
# `command too long`。
#
# 危險的地方在於**它是靜默的**：tmux 回非零，但呼叫端如果不看回傳值，
# 就會回報「已追加」而內容其實一個位元組都沒動。
# 實測過：16201 bytes 的一則追加 600 bytes，MCP 回「已追加」，內容沒變。
# 這個系統沒有版本歷史，所以那段字就是永久消失。
ab_set_prompt() {          # $1 = window_id，$2 = 內容的檔案路徑
    tmux set-option -w -t "$1" "$K_PROMPT" "$(cat "$2")" 2>/dev/null
}

# 那則現在多大（bytes）。給錯誤訊息用，讓 agent 知道還剩多少空間。
ab_prompt_size() {
    ab_prompt "$1" | LC_ALL=C wc -c | tr -d ' '
}
