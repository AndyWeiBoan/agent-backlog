...
- **UI 的「非互動窗格」要防止焦點落進去**。預覽窗格只是個 `cat` 迴圈，不讀鍵 ——
  焦點一旦跑過去（滑鼠點、`prefix o`、`prefix 方向鍵`…），使用者打的字會被 tty
  直接回顯在畫面上，ESC / Enter 全部沒反應，看起來就是「卡死了只能 C-c」。
  擋來源擋不完，改成掛 `window-pane-changed` hook「跑過去就彈回來」，
  再讓那個窗格 `stty -echo` 當第二層保險
- **清單變空時，預覽也要主動清掉**。「沒有選中項就 early return」會讓畫面停在上一則，
  看起來像還選著它。而且**不能用「上次畫的是誰」當判斷依據** —— 切換範圍、重整、
  resize 都會把那個變數清掉來強制重畫，那樣就分不出「畫面上還有沒有東西」。
  要另外用一個旗標記錄預覽目前是不是空的
